 ...
 * @author Usuario
 */
public class SegundaFase extends GameView
{
    ...
    public static SegundaFase getInstance(int gameType, KeyListener listener)
    {
        if (instancia == null) {
            instancia = new SegundaFase(gameType, listener);
        }
        
        return instancia;
    }

    private SegundaFase(int gameType, KeyListener listener)
    {
        super(gameType);
        addKeyListener(listener);
        setFocusable(true);
        
        proximaFase = UltimaFase.getInstance(gameType, listener);
    }   
    ...
    public void setProximaFase()
    {
        ...
        game.setEnemys(factory.criarInimigosFortes(8));
        game.setBackgroundGame(factory.criarCenarioFase2());
        game.setEscudos(factory.criarEscudosFase3());
    }
    
    public static void show(int gameType, Game game, KeyListener listener)
        throws InterruptedException
    {
        SegundaFase.getInstance(gameType, listener)
            .jogar(game);
    }
}
